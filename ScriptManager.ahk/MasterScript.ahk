...
tray_icons()
{
	...
	Loop, Parse, trayWindows, |
	{
		...
		SendMessage, 0x0418, 0, 0, ToolbarWindow32%idxTB%, ahk_class %A_LoopField%	; 0x0418 = TB_BUTTONCOUNT
		Loop, %ErrorLevel%
		{
			SendMessage, 0x0417, A_Index - 1, pProc, ToolbarWindow32%idxTB%, ahk_class %A_LoopField%	; 0x0417 = TB_GETBUTTON
			...
			idx	:= A_Index - 1, toolTip := A_IsUnicode ? wTooltip : sTooltip, index++
			For a, b In array[0]
				array[index, b] := %b%
		}
